...
export class TheFinManBot {
  ...
  async hear() {
    ...
    this.bot.command('privacidade', ctx => {
      ctx.reply('O FinMan é um software livre, gratuito e de código aberto que não armazena nenhuma informação pessoal alem do email do comprador e o ID do grupo do Telegram vinculado a ele. Todos seus gastos são armazenados em uma planilha online de sua propriedade em você pode a qualquer momento impedir que eu leia ou edite a mesma. O Telegram usa criptografia para garantir que ninguém alem dos integrantes do grupo leiam suas mensagens!\n\nSaiba mais usando os botoes abaixo!', {
        parse_mode: 'Markdown',
