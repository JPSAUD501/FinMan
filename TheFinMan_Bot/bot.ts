...
export class TheFinManBot {
  ...
  async hear() {
    ...
    this.bot.command('privacidade', ctx => {
      ctx.reply('O FinMan é um software livre, gratuito e de código aberto que não armazena nenhuma informação pessoal alem do email do comprador e o ID do grupo do Telegram vinculado a ele. Todos seus gastos são armazenados em uma planilha online de sua propriedade em você pode a qualquer momento impedir que eu leia ou edite a mesma. O Telegram usa criptografia MTProto para garantir que ninguém alem dos integrantes do grupo leiam suas mensagens!', {
        parse_mode: 'Markdown',
        ...Markup.inlineKeyboard([
          Markup.button.url('Código Aberto', 'https://pt.wikipedia.org/wiki/Software_de_c%C3%B3digo_aberto'),
          Markup.button.url('MTProto', 'https://core-telegram-org.translate.goog/mtproto?_x_tr_sl=en&_x_tr_tl=pt&_x_tr_hl=pt-BR&_x_tr_pto=wapp')
        ])
      });
    })
